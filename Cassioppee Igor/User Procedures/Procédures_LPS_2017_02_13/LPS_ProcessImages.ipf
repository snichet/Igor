...
Function transcribe(wave_in)
wave wave_in
Make/O/N=(DimSize(wave_in,1),DimSize(wave_in,0),DimSize(wave_in,2)) tmp_3D_raw 
SetScale/P x DimOffset(wave_in,1),DimDelta(wave_in,1),"slit_angle", tmp_3D_raw
SetScale/P y DimOffset(wave_in,0),DimDelta(wave_in,0),"energy", tmp_3D_raw
SetScale/P z DimOffset(wave_in,2),DimDelta(wave_in,2),"other_angle", tmp_3D_raw
tmp_3D_raw=wave_in[q][p][r]
end
...
variable pstart,qstart,rstart
	...
	variable transpose=0 	// 0 if x=angle and y=energy
						// 1 if x=energy and y=angle
	variable dim_angle,dim_energy
	...
	OldXstart=round(DimOffset($InputWaveName,dim_angle)*100000)/100000
	OldYstart=round(DimOffset($InputWaveName,dim_energy)*100000)/100000
	pstart=round((cropstart-OldXstart)/DimDelta($InputWaveName,dim_angle))
	qstart=round((energystart-OldYstart)/DimDelta($InputWaveName,dim_energy))
	...
	NbX=round((cropend-cropstart)/DimDelta($InputWaveName,dim_angle))+1
	NbY=abs(round((energyend-energystart)/DimDelta($InputWaveName,dim_energy)))+1
	NbZ=round((Zend-Zstart)/DimDelta($InputWaveName,2))+1
	
	Make/O /N=(NbX,NbY,NbZ) temp3D
	Duplicate/O $InputWaveName Old3D
	SetScale/P x cropstart,DimDelta($InputWaveName,dim_angle), temp3D
	SetScale/P y energystart-ZeroFermi,DimDelta($InputWaveName,dim_energy), temp3D
	SetScale/P z Zstart,DimDelta($InputWaveName,2), temp3D
	
	if (transpose==0)
		temp3D=Old3D[p+pstart][q+qstart][r+rstart]
		else
		temp3D=Old3D[q+qstart][p+pstart][r+rstart]
	endif
	...
	SetScale/P x cropstart,DimDelta($InputWaveName,dim_angle)*AvgX, temp3D
	SetScale/P y energystart-ZeroFermi,DimDelta($InputWaveName,dim_energy)*AvgY, temp3D
